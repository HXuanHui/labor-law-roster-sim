      ...
      <div className="bg-white border border-[#E9E7D4] rounded-2xl max-w-2xl w-full p-6 shadow-xl text-[#2D2D2D] space-y-5 my-8">
        <div className="flex items-center justify-between border-b border-[#E9E7D4] pb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-lg font-bold text-[#2D2D2D] font-serif">班別時間與類別設定</h2>
              <p className="text-xs text-[#8A8A70]">可自訂或編輯早/中/夜班、長班與休息日之上下班時間、工時與代表色彩</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleAdd} className="bg-[#F8F7EB] p-4 rounded-xl border border-[#E9E7D4] space-y-3">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            <div>
              <label className="block text-xs text-[#8A8A70] mb-1">班別簡碼 (例：早)</label>
              <input
                type="text"
                maxLength={3}
                value={code}
            ...
            <div>
              <label className="block text-xs text-[#8A8A70] mb-1">班別名稱</label>
              <input
                type="text"
                value={name}
            ...
            <div>
              <label className="block text-xs text-[#8A8A70] mb-1">屬性類別</label>
              <select
                value={category}
                onChange={(e) => setCategory(e.target.value as any)}
                className="w-full bg-white border border-[#D9D7C2] rounded-xl px-2 py-2 text-xs text-[#2D2D2D] outline-none focus:ring-2 focus:ring-[#5A5A40]"
              >
            ...
            <div>
              <label className="block text-xs text-[#8A8A70] mb-1">代表色彩</label>
              <input
                type="color"
                value={color}
          ...
          {category === 'work' && (
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 pt-1 border-t border-[#E9E7D4]/60">
              <div>
                <label className="block text-xs text-[#8A8A70] mb-1">上班時間</label>
                <input
                  type="time"
                  value={startTime}
                  onChange={(e) => handleStartTimeChange(e.target.value)}
                  className="w-full bg-white border border-[#D9D7C2] rounded-xl px-2 py-1.5 text-xs text-[#2D2D2D] outline-none focus:ring-2 focus:ring-[#5A5A40] font-mono"
                />
              </div>

              <div>
                <label className="block text-xs text-[#8A8A70] mb-1">下班時間</label>
                <input
                  type="time"
                  value={endTime}
              ...
              <div>
                <label className="block text-xs text-[#8A8A70] mb-1">休息時間 (小時)</label>
                <input
                  type="number"
                  step="0.5"
                  min="0"
              ...
              <div>
                <label className="block text-xs text-[#8A8A70] mb-1">每日工時 (小時)</label>
                <input
                  type="number"
                  step="0.5"
                  min="0"
                  ...
                  className="w-full bg-white border border-[#D9D7C2] rounded-xl px-2 py-1.5 text-xs text-[#2D2D2D] outline-none focus:ring-2 focus:ring-[#5A5A40] font-mono font-bold text-[#5A5A40]"
                />
              </div>
            </div>
          )}
        ...
        <div className="space-y-2">
          <div className="text-xs font-bold text-[#8A8A70]">班別與時間清單（點擊編輯圖示可調整時間）</div>
            ...
            {shiftTypes.map((st) => {
              ...
              if (isEditing) {
                return (
                  ...
                  >
                    <div className="text-xs font-bold text-[#5A5A40] flex items-center justify-between border-b pb-1.5">
                      <span>編輯班別：{st.name}</span>
                      <span className="text-[10px] text-[#8A8A70] font-mono">ID: {st.id}</span>
                    </div>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                      ...
                      <div>
                        <label className="block text-[10px] text-[#8A8A70] mb-0.5">班別全名</label>
                        <input
                          type="text"
                          value={editForm.name || ''}
                      ...
                      <div>
                        ...
                        <input
                          ...
                          className="w-full bg-[#F8F7EB] border border-[#D9D7C2] rounded-lg px-2 py-1 text-xs font-mono font-bold"
                        />
                      </div>
                    </div>
              ...
              return (
                <div
                  key={st.id}
                  className="bg-[#F8F7EB] p-3 rounded-xl border border-[#E9E7D4] flex items-center justify-between text-xs"
                >
                  <div className="flex items-center space-x-3">
                    <div
                    ...
                    <div>
                      <div className="font-semibold text-[#2D2D2D] flex items-center gap-2">
                        <span>{st.name}</span>
                        {st.category === 'work' && (
                          <span className="text-[10px] text-[#5A5A40] font-mono bg-[#5A5A40]/10 px-1.5 py-0.5 rounded flex items-center gap-1">
